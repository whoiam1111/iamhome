...
export default function ResultView({
	...
	const mainTypeNames = firstMains
		.map((m) => DANDAN_RESULTS[m.type].typeName)
		.join(" / ");
	const secondTypeNames =
		secondMains.length > 0
			? secondMains.map((m) => DANDAN_RESULTS[m.type].typeName).join(" / ")
			: "없음";
		...
		<div className="w-full flex flex-col items-center animate-in fade-in duration-500 pb-20 pt-10">
			...
			<div className="w-full max-w-4xl bg-neutral-900 text-white rounded-3xl p-8 md:p-12 shadow-xl mb-8 relative overflow-hidden">
				...
				<div className="relative z-10">
					<p className="text-blue-400 font-bold tracking-wide mb-2">
						나의 1차 성향
					</p>
					<h1 className="text-4xl md:text-5xl font-black mb-4">
						<span className="text-white">{userName}</span>님은{" "}
						<span className="text-blue-500">{mainTypeNames}</span> 입니다.
					</h1>
					{secondMains.length > 0 && (
						<p className="text-xl text-gray-300 font-medium">
							2차(보조) 성향은{" "}
							<span className="text-white font-bold">{secondTypeNames}</span>{" "}
							요소가 강하게 나타납니다.
						</p>
					)}
					...
					<div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-10">
						{sorted.map((item) => (
							<div
								key={item.type}
								className="bg-white/10 rounded-2xl p-4 flex flex-col justify-center items-center"
							>
								<span className="text-sm font-bold text-gray-400 mb-1">
									{DANDAN_RESULTS[item.type].typeName}
								</span>
				...
				<div className="w-full lg:w-2/3 flex flex-col gap-6">
					{firstMains.map((main) => {
						...
						return (
							<div
								key={main.type}
								className="w-full bg-white rounded-3xl shadow-md p-8 md:p-10 border border-gray-100"
							>
								...
								<h3 className="text-xl font-bold text-neutral-500 mb-6 pb-6 border-b border-gray-100">
									&quot;{data.subtitle}&quot;
								</h3>
								...
								<div className="relative mt-8">
									...
									<div className="blur-md select-none pointer-events-none opacity-30">
											...
											<div>
												...
												<ul className="flex flex-col gap-3">
													{data.strengths.map((s, idx) => (
														<li
															key={idx}
															className="flex items-start gap-2 text-neutral-600 break-keep"
														>
										...
										<div className="bg-neutral-50 rounded-2xl p-6 md:p-8 space-y-6">
											<div>
												...
												<div className="text-neutral-800 font-medium break-keep">
													{data.motivation}
												</div>
											</div>
											<div>
									...
									<div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center z-10">
										<div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-lg mb-4 text-neutral-700">
											<LockClosedIcon className="w-8 h-8" />
										</div>
										<p className="text-xl md:text-2xl font-bold text-neutral-800 tracking-tight bg-white/80 px-6 py-2 rounded-full shadow-sm backdrop-blur-sm">
											자세한 내용은 프로그램을 통해 확인하세요
										</p>
									</div>
								</div>
							</div>
				...
				<div className="w-full lg:w-1/3 flex flex-col gap-6">
					{secondMains.map((second) => {
						...
						return (
							<div
								key={second.type}
								className="w-full bg-white rounded-3xl shadow-sm p-6 md:p-8 border border-gray-100 flex flex-col h-full"
							>
								<div className="inline-flex items-center justify-center px-3 py-1 bg-gray-100 rounded-full mb-4 self-start">
									<span className="text-gray-600 font-bold text-xs tracking-wide">
										2차 보조 성향
									</span>
								</div>
								...
								<p className="text-neutral-600 text-sm leading-relaxed mb-6 pb-6 border-b border-gray-100 break-keep">
									<strong className="text-neutral-800">{userName}</strong>님은
									기본적으로 {mainTypeNames}의 특성을 가지면서도 언제든 이{" "}
									{data.typeName}의 강점이{" "}
									<strong className="text-neutral-800">{userName}</strong>님을
									뒷받침해 줍니다.
								</p>

								<div className="relative flex-1 mt-4">
									{/* 블러 처리되는 실제 컨텐츠 */}
									<div className="blur-md select-none pointer-events-none opacity-30 space-y-4">
										<div>
											<div className="text-xs font-bold text-blue-500 mb-1">
												보완되는 장점
											</div>
											<div className="text-neutral-700 text-sm font-medium break-keep">
												{data.strengths[0]}
											</div>
										</div>
										<div>
									...
									{/* 잠금 오버레이 */}
									<div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center z-10 min-h-[120px]">
										<div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-md mb-3 text-neutral-600">
											<LockClosedIcon className="w-6 h-6" />
										</div>
										<p className="text-xs font-bold text-neutral-800 bg-white/90 px-4 py-1.5 rounded-full shadow-sm backdrop-blur-sm">
											자세한 내용은 프로그램을 통해 확인하세요
										</p>
									</div>
								</div>
